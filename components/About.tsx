      ...
      >
        {skills.map((skill, i) => (
          <motion.div
            ...
            className="p-6 bg-dark-800/30 border border-white/10 rounded-xl backdrop-blur-md backdrop-saturate-150 shadow-lg shadow-black/20"
          >
            <div className="text-accent mb-4">{skill.icon}</div>
            <h3 className="text-lg font-bold mb-3">{skill.name}</h3>
            <div className="flex flex-wrap gap-2">
              {skill.items.map((item) => (
                <span key={item} className="px-2 py-1 text-xs bg-white/5 border border-white/10 rounded text-gray-300 backdrop-blur-sm">{item}</span>
              ))}
            </div>
          </motion.div>
        ))}

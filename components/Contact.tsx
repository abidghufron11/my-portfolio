...
export default function Contact() {
  ...
  const scrollProps = isMobile ? {} : { initial: "hidden", whileInView: "visible", viewport: { once: true } };

  return (
    <section id="contact" className="py-24 px-6 max-w-6xl mx-auto">
      <motion.div variants={fadeInUp} {...scrollProps} className="text-center mb-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        <motion.div variants={fadeInUp} {...scrollProps} className="space-y-8">
          ...
          {status.type && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`p-4 rounded-lg flex items-center gap-3 text-sm ${status.type === "success" ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-red-500/10 text-red-400 border border-red-500/20"}`}>
        ...
        <motion.form variants={fadeInUp} {...scrollProps} className="space-y-5" onSubmit={handleSubmit}>
          {["name", "email"].map((field) => (
            <div key={field} className="relative">
              <input name={field} type={field === "email" ? "email" : "text"} placeholder={field.charAt(0).toUpperCase() + field.slice(1)} required onFocus={() => setFocused(field)} onBlur={() => setFocused(null)} disabled={loading} className={`w-full bg-dark-800/50 border ${focused === field ? "border-accent" : "border-white/10"} rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-accent/50 transition-all placeholder:text-gray-500 text-white disabled:opacity-50`} />
          ...
          <div className="relative">
            <textarea name="message" placeholder="Message" required rows={6} onFocus={() => setFocused("message")} onBlur={() => setFocused(null)} disabled={loading} className={`w-full bg-dark-800/50 border ${focused === "message" ? "border-accent" : "border-white/10"} rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-accent/50 transition-all placeholder:text-gray-500 text-white resize-none disabled:opacity-50`} />
          ...
          <motion.button type="submit" variants={buttonHover} whileHover="hover" whileTap={!isMobile ? { scale: 0.96 } : {}} className="relative overflow-hidden w-full px-6 py-3.5 bg-accent text-black font-semibold rounded-lg flex items-center justify-center gap-2 cursor-pointer">
            <motion.div className="absolute inset-0 bg-gradient-to-r from-orange-400 to-accent z-0" variants={{ hover: { scaleX: 1 } }} initial={{ scaleX: 0 }} transition={{ duration: 0.4, ease: "easeInOut" }} style={{ originX: 0 }} />
            <span className="relative z-10">{loading ? <><Loader2 className="animate-spin" size={16} /> Sending...</> : "Send Message"}</span>
            {!loading && <motion.span className="relative z-10" variants={{ hover: { x: 4 } }} transition={{ type: "spring", stiffness: 300 }}><Send size={16} /></motion.span>}
          </motion.button>
        </motion.form>
      </div>
    </section>
